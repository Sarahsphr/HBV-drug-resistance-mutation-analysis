# count_S_mutations.py
# Counts known S protein mutations per genotype from aligned FASTA files
...
#PATH TO YOUR S ALIGNMENTS
ALIGN_DIR = Path("data/Alig_S")
#OUTPUT CSV
OUTPUT_CSV = ALIGN_DIR / "S_mutation_counts.csv"
    ...
    for part in parts:
        ...
        if not m:
            raise ValueError(
                ...
                f"(expected like G145R or G145E+C147Y)"
            )
...
def has_mutation_direct(seq_aln: str, mut_parts) -> bool:
    ...
    Example:
      G145R means count if position 145 is R.
...
def main():
    ...
    files = sorted(ALIGN_DIR.glob("GT*_alig_S*.fas*"))

    if not files:
        raise FileNotFoundError(
            f"No S alignment files found in {ALIGN_DIR} "
            f"matching GT*_alig_S*.fas*"
        )
